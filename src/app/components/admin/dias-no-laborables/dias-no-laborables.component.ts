...
@Component({
  selector: 'app-dias-no-laborables',
  templateUrl: './dias-no-laborables.component.html',
  styleUrls: ['./dias-no-laborables.component.css']
})
export class DiasNoLaborablesComponent implements OnInit , AfterViewChecked {
  ...
  constructor(private _fechaService:FechasService , private spinner:NgxSpinnerService ,  private elementRef: ElementRef ,private funcionesP: FuncionesJSService ) { this.funciones = funcionesP; }
  ...
  guardar(forma : NgForm){
    ...
    this._fechaService.postCrearFechaNoHabil(this.fechaNoHabil).subscribe(data=>{
      if (data['success']) {
  ...
  editar(obj) {
   ...
   editarGuardar(forma : NgForm){
    ...
    this._fechaService.postEditarFechaNoHabil(this.fechaNoHabil).subscribe(data=>{
  ...
  estadoDato(obj) {
    ...
    swal({
      ...
      if (res.value) {
        this.spinner.show();
        this._fechaService.postEstadoDato(obj).subscribe(data => {
